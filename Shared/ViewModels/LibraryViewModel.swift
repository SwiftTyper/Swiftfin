...
final class LibraryViewModel: PagingLibraryViewModel {
    ...
    private func requestItems(with filters: ItemFilters, replaceCurrentItems: Bool = false) {
        ...
        if filters.filters.contains(ItemFilter.isFavorite.filter) {
            includeItemTypes = [.movie, .boxSet, .series, .season, .episode, .musicVideo, .musicAlbum]
        } else if type == .folders {
            recursive = false
            includeItemTypes = [.movie, .boxSet, .series, .folder, .collectionFolder, .musicAlbum, .musicVideo, .musicGenre, .musicArtist]
        } else {
            includeItemTypes = [.movie, .boxSet, .series, .musicVideo, .musicAlbum]
        }
